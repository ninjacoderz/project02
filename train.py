...
def set_args():
    ...
    parser.add_argument('--arch', default='densenet',
                        help='pretrained model architectures, options: vgg, densenet, alexnet')
    parser.add_argument(
        '--hidden_units', default=[512], type=int, nargs='*', help='hidden layer architecture')
    ...
    parser.add_argument('--gpu', default='mps', type=str,
                        help='devices, options: gpu, cpu, mps')
...
def load_data(data_dir):
    ...
    dataloaders = {
        x: torch.utils.data.DataLoader(image_datasets[x], batch_size=32, shuffle=True) for x in
        ['train', 'val', 'test']
    }
...
def main():
    ...
    model, accuracy = train_model(
        model, criterion, optimizer, args.epochs, args.gpu, dataloaders)
    save_model(image_datasets, model, optimizer,
               criterion, args.epochs, args.save_dir)
